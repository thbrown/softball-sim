...
public class MonteCarloExhaustiveTask implements Callable<TaskResult> {
  ...
  public MonteCarloExhaustiveTask(BattingLineup lineup, long numberOfGamesToSimulate, int inningsPerGame,
  ...
  public TaskResult run() {
    ...
    for (int i = 0; i < numberOfGamesToSimulate; i++) {
      ...
      for (int inning = 0; this.inningsPerGame > inning; inning++) {
        ...
        while (outs < 3 && runsThisInning < MAX_RUNS_PER_INNING) {
          ...
          batterIndex++;
          int bases = hitGenerator.hit(p.getId());
          if (bases > 0) {
    ...
    TaskResult result = new TaskResult(score, lineup);
    return result;
  }
